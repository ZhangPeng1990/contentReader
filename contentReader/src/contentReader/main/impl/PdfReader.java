...
public class PdfReader implements Reader {
	...
	public PdfReader(File file) {
		...
		try {
			...
			txtFile = this.readFdf(file.getAbsolutePath());
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public Integer getWordNum() {
		if(txtFile != null){
			Reader reader = new TxtReader(txtFile);
			int num = reader.getWordNum();
			txtFile.delete();
			return num;
		}
		return null;
	...
	private File readFdf(String file) throws Exception {
		// 是否排序
		boolean sort = false;
		// pdf文件名
		String pdfFile = file;
